#!/usr/bin/env python3
"""
Script batch per processar tots els PDFs de la Lliga de Promoció 2009-2010.
...
Fonts:
  https://old.fcatletisme.cat/Promocio/promocio2010/calendaripromocio.htm
  https://old.fcatletisme.cat/Promocio/promocio2010/lligapromocio/jornades.html
"""
...
# Page URLs and their base directories for 2009-2010 season
# Calendar page (Excel frameset - needs sheet001.htm scraping)
# Jornada page (regular HTML with PDF links)
PAGES = [
    ("https://old.fcatletisme.cat/Promocio/promocio2010/calendaripromocio.htm",
     "https://old.fcatletisme.cat/Promocio/promocio2010/"),
    ("https://old.fcatletisme.cat/Promocio/promocio2010/lligapromocio/jornades.html",
     "https://old.fcatletisme.cat/Promocio/promocio2010/lligapromocio/"),
]
...
def extract_pdf_links(html, base_url, page_url):
    """Extract PDF links from HTML content, resolving relative URLs.
    
    For Excel frameset pages (calendar), also fetch the sheet001.htm sub-page.
    """
    if not html:
        return []
    ...
    # For Excel frameset pages, also scrape the sheet content
    if 'calendaripromocio.htm' in page_url or 'calendaripromocio_archivos' in page_url:
        sheet_url = page_url.replace('.htm', '_archivos/sheet001.htm')
        sheet_html = fetch_url(sheet_url)
        if sheet_html:
            sheet_links = re.findall(r'href=["\']([^"\']*\.pdf[^"\']*)["\']', sheet_html)
...
def process_pdf(pdf_path, json_dir, pdf_url=""):
    ...
    try:
        cmd = ["python3", "extract_promocio.py", pdf_path, json_dir]
        if pdf_url:
            cmd.append(pdf_url)
        ...
        # Count results from output - match "Found X CATT result entries" or "After validation: X valid results"
        total = 0
        for line in result.stdout.split('\n'):
            m = re.search(r'Found\s+(\d+)\s+CATT\s+result', line)
            if m:
                total = int(m.group(1))
            else:
                m = re.search(r'After validation:\s+(\d+)\s+valid\s+result', line)
                if m:
                    total = int(m.group(1))
...
def is_result_pdf(url):
    """Check if a URL is a results PDF (not regulations, schedules, etc.)."""
    # Skip non-result PDFs
    skip_patterns = [
        'regl', 'regla', 'reglliga', 'taulespuntuacio',  # regulations
        'horari', 'horaris',  # schedules
        'file://',  # local network paths
        'rfea.es',  # external RFEA results
        'championchip',  # external site
    ]
    url_lower = url.lower()
    for pattern in skip_patterns:
        if pattern in url_lower:
            return False
    # Must contain "resul" or "result"
    return 'resul' in url_lower or 'result' in url_lower


def main():
    print("=" * 60)
    print("PROCESSING PROMOCIO 2009-2010 RESULTS")
    print("=" * 60)
    ...
    for page_url, base_url in PAGES:
        ...
        if html:
            pdfs = extract_pdf_links(html, base_url, page_url)
            # Filter to result PDFs only
            result_pdfs = [p for p in pdfs if is_result_pdf(p)]
